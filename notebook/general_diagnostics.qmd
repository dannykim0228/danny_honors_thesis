...
```{python}
PKL_PATH = "../output/1d_global/search_13/1d_global_out.pkl"
...
LL_frame.reset_index().to_csv("notebook/LL_frame.csv", index = False)
```
...
```{r}
traces <- read.csv("traces.csv")
LL_frame = read.csv("LL_frame.csv")
...
# Parameter estiamtes for the best fit
traces |>
...
```{r}
ggplot(
    filter(traces_pyr, quantity != "logLik"), 
    aes(x = iter, y = value_T, group = interaction(rep, language), color = as.factor(language))
) + 
    geom_line() + 
    facet_wrap(vars(quantity), scales = "free")
...
traces_pyr |>
    group_by(language) |>
    filter(iter == max(iter)) |>
    ggplot(mapping = aes(x = language, y = value_T, color = language)) +
    geom_jitter() +
    facet_wrap(vars(quantity), scales = "free") +
    labs(
        title = "Comparison of Final Iteration Values for Each Parameter",
        x = "Parameter",
        y = "Transformed Value",
        fill = "Language"
    ) +
