...
sp500_rinit <- "
  // V = 7.86e-3 * 7.86e-3; // V_0 is a parameter as well
  V = V_0;
...
sp500_Nreps_eval <-   switch(run_level,   3,  7,   10,  24)
sp500_Nreps_global <- switch(run_level,  5,  15,  20, 120)
...
write.csv(data.frame(), "initial_params_r.csv", row.names = FALSE)
write.csv(global_starts, "initial_params_r.csv", row.names = FALSE)
cat("Saved", nrow(global_starts), "initial parameter sets to initial_params_r.csv\n")
...
stew(file = sprintf("1d_global_search.rda"), {
  t.box <- system.time({
